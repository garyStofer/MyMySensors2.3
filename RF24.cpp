...
{
	uint8_t RF24_status;
	uint16_t timeout = 0xFFFF;	// timeout counter to detect missing HW issues
	...
	do 
	{
		...
		RF24_status = hwDigitalRead(MY_RF24_IRQ_PIN);		// polling the IRQ line from the radio to see if sending finished, not polling via SPI, in order to keep things quiet during transmission 
#else
		RF24_status = ! (RF24_getStatus() & (_BV(RF24_MAX_RT) | _BV(RF24_TX_DS)));  // IRQ line is not connected -- have to poll via SPI bus
#endif	
	} while  (RF24_status && timeout--);							// Device indicated that transmission is complete  
	
	
	RF24_ce(LOW);		//transmission ends

	RF24_status = RF24_setStatus(_BV(RF24_RX_DR) | _BV(RF24_TX_DS) | _BV(RF24_MAX_RT));  // get Status and reset interrupt indications
	...
	if (timeout == 0 )
	{
		RF24_DEBUG(PSTR("?RF24:TXM:MAX_TIMEOUT\n"));	// max timeout reached 
		RF24_flushTX();
	}
		
	if (noACK) {		// ?? set retries back for next time ???
